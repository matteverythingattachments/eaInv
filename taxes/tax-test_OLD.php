...
*/
  ...
  */
  ...
  if(!in_array($orderid, $orders_arr)):
    ...
    $total_tax = $total_tax + (int)$salestax1;
    $total_pay = $total_pay + (int)$totalpayment;
    ...
    $table_data .= '<td>'.$customerid.'</td><td>'.$orderdate.'</td><td>'.$paymentdate.'</td><td>'.$shipstate.'</td><td>'.$shipcity.'</td><td rel="'.$shippostalcode.'"><a add target="_blank" href="http://www.sale-tax.com/'.$shipzip.'">'.$shipzip.'</a></td><td>'.money_format('%.2n', $total_wo_tax).'</td><td>'.money_format('%.2n', $salestax1).'</td><td>'.$useTaxRate.'%</td><td>'.money_format('%.2n', $totalpayment).'</td></tr>';
...
<body>
 ...
 <table id="frighttable" style="margin-top:30px;">
   <tr><th>#</th><th>Order ID</th><th>Status</th><th>Inv Date</th><th>Pay Date</th><th>State</th><th>City</th><th>Postal Code</th><th>Net</th><th>Tax</th><th>Rate</th><th>Gross Payment</th></tr>
   
   <?php echo $table_data; ?>
   <tr><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td><?php echo  money_format('%.2n', $totaltaxamount); ?></td><td></td><td><?php echo money_format('%.2n', $total_pay); ?></td></tr>
  </table>
  <br /><br />
