...
if (!empty($_GET['getinfo']) && !empty($_GET['orderid']) && ($_SESSION['loggedin'] == "admin")) :
  ...
  //https://www.everythingattachments.com/net/WebService.aspx?Login=production@everythingattachments.com&EncryptedPassword=ACA051D7C73CFE239E145A6BC6F997D3CAC3819518FDD0D28AFC27C2067DC081&API_Name=Generic\Orders&SELECT_Columns=od.OrderID,od.ProductName,od.ProductNote,od.Options,od.CustomLineItem,o.OrderID,o.BillingFirstName,o.BillingLastName,o.LastModified,o.OrderDate,o.OrderStatus,o.PaymentAmount,o.Shipped,o.ShipState,o.OrderNotes,o.Order_Comments,o.Total_Payment_Received&WHERE_Column=o.OrderID&WHERE_Value=
...
if (!empty($_POST['savedata']) && ($_SESSION['loggedin'] == "admin" || $_SESSION['loggedin'] == "nate")) :
  ...
  if (is_array($post_product_array)) :
    foreach ($post_product_array as $key => $aproduct) :
      ...
      if (!empty($_POST['data_id'])) :
        ...
        $updatethe_product = mysqli_query($conn, $updatequery_prodcuts) or die(mysqli_error($conn));
        if (!$updatethe_product) : $error = "Crap something went wrong!! Try again in a few minutes or call Scott";
        endif;
        ...
        if ($row && $key !== "Not-Found") :
          ...
          if (!empty($set_received_date)) :
            $addproduct = "INSERT INTO production (request_date, order_id, product_code, customer, product, note,cut,top,bottom,welded,painted,pallet,received,hide,edit) VALUES('$format_date','$post_orderid','$key','$post_customername','$aproduct',' ', '0000-00-00','0000-00-00','0000-00-00','0000-00-00','0000-00-00','0000-00-00','0000-00-00','$format_received_date','0','0')";
            $addnewproduct = mysqli_query($conn, $addproduct) or die(mysqli_error($conn));
            if (!$addnewproduct) : $error = "Crap something went wrong!! Try again in a few minutes or call Scott";
            endif;
          else :
            // ADD NEW
            $addproduct = "INSERT INTO production (request_date, order_id, product_code, customer, product, note,cut,top,bottom,welded,painted,pallet,received,hide,edit) VALUES('$format_date','$post_orderid','$key','$post_customername','$aproduct',' ', '0000-00-00','0000-00-00','0000-00-00','0000-00-00','0000-00-00','0000-00-00','0000-00-00','0','0')";
            $addnewproduct = mysqli_query($conn, $addproduct) or die(mysqli_error($conn));
            if (!$addnewproduct) : $error = "Crap something went wrong!! Try again in a few minutes or call Scott";
            endif;
          endif;
  ...
  if ($_SESSION['loggedin'] == "nate" && isset($_POST['data_id'])) :
    ...
    $updatethe_product = mysqli_query($conn, $updatequery_prodcuts) or die(mysqli_error($myConnection));
    if (!$updatethe_product) : $error = "Crap something went wrong!! Try again in a few minutes or call Scott";
    endif;
  else :
    $saved = "yes";
...
<body>
  ...
  if (!empty($_GET['complete'])) :
    ...
    $countplacement_query = mysqli_query($conn, "SELECT * FROM production WHERE received = '0000-00-00' ORDER BY order_id") or die(mysqli_error($conn));
  ...
  while ($data = mysqli_fetch_assoc($get_data)) :
    ...
    if ($data['cut'] === "0000-00-00") :
      ...
      $data_cut   = date("m/d/Y", strtotime($data['cut']));
    endif;
    ...
    // GET TOTAL NUMBER OF PRODUCT COUNT FOR A CERTAIN PRODUCT CODE
    //if ($test_result1 = $conn->query("SELECT * FROM production WHERE received = '0000-00-00' AND product_code = '".$data_product_code."' ORDER BY request_date")) {
    //   $row_cnt1 = $test_result1->num_rows;
    //   $test_result1->close();
    // } 

    // GET # OUT OF TOTAL OF THESE TYPE OF PRODUCTS 
    // if ($test_result2 = $conn->query("SELECT * FROM production WHERE received = '0000-00-00' AND product_code = '".$data_product_code."' AND id <= '".$data_id."' ORDER BY order_id")) {
    ...
    // GET # OUT OF TOTAL OF THESE TYPE OF PRODUCTS 
    if (is_array($getting_count[$data_product_code])) :
    ...
    $getnum2 = count((array)$getting_count[$data_product_code]);


    if ($data_product_code !== "Not-Found") :
      $product_data = '<a href="https://www.everythingattachments.com/ProductDetails.asp?ProductCode=' . $data_product_code . '" class="productlink" target="_blank" title="Number ' . $getnum1 . ' out of ' . $getnum2 . ' to be built">' . $data_product . '</a> ';
    else :
    ...
    if (!empty($_SESSION['loggedin']) && ($_SESSION['loggedin'] == 'admin' || $_SESSION['loggedin'] == 'nate')) :
      ...
      $tabledata .= '<td><a href="https://www.everythingattachments.com/admin/AdminDetails_ProcessOrder.asp?table=Orders&Page=1&ID=' . $data_order_id . '" target="_blank" class="productlink" title="' . $showcount . ' out of ' . $row_cnt0 . ' in total">' . $data_order_id . '</a></td>';
    endif;
  ...
  <div id="tabledatadiv">
    <table id="tabledata">
      <tr class="fixed">
        <th width="93">Request Date</th>
        <th width="62">Order ID</th>
        <th width="65">Customer</th>
        <th width="185" id="productcell">Product&nbsp;&nbsp;&nbsp;<?php echo '  (' . ($showcount - 1) . ' Items)'; ?><?php if (!empty($_SESSION['loggedin']) && $_SESSION['loggedin'] == 'admin') : ?>
          <form method="post" id="getorderinfoform">
            <input type="hidden" id="data_id" name="data_id" placeholder="Order ID#" value="" />
            <input type="text" id="orderid" name="orderid" style="width:100px;" value="" />
            <input type="submit" id="getorderinfo" value="Get Info" />
          </form>
        <?php endif; ?>
        </th>
        <th width="132"> Notes <a href="other-products.php" title="Non-Freight Products" style="padding: 0 5px 0 5px;color:#ffeb3b;">Non-Freight Products</a></th>
        <th width="103"> Tack </th>
        <?php if (!empty($_GET['complete'])) : ?>
          <th width="63" class="center topth">Received</th>
          <th width="87" class="center topth">Days Waited</th>
        <?php else : ?>
          <th width="91" class="center topth">Days Waiting</th>
        <?php endif; ?>
      ...
      <tr>
        ...
        <td></td>
        <td></td>
        <td class="center" id="cellsize"></td>
        <td></td>
        <td></td>
        <td></td>
        <?php if (!empty($_GET['complete'])) : ?><td></td>
  ...
  <div id="bottombar">
    <a href="index.php" <?php if (empty($_GET['complete'])) {
                          echo 'class="selected"';
                        } ?> title="Factory Product Schedule LTL">Factory Product Schedule LTL</a>
    <a href="index.php?complete=1" <?php if (!empty($_GET['complete'])) {
                                      echo 'class="selected"';
                                    } ?> title="Complete Factory to Assembly">Complete Factory to Assembly</a>
    <a href="charles.php" title="Charles List">Charles</a>

    <div id="zipdiv">
      <input type="text" id="zipcode" value="Zip Code" />
      <input type="button" id="taxlookup" value="Tax Look Up" />
    </div>
