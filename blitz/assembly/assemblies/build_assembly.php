...
// check the task parameter to make sure the appropriate actions are taken
	...
	mysqli_query($mysqli,"INSERT INTO assembly_build (PartID,assemblyID,Qty,Type) VALUES('$_GET[subassembly_num]','$_GET[ID]','$_GET[qty]','SA')");
	getBlaster();
	}
...
<?php 
    $color = '#FF2020'; //Green: '#1AC91A'
    $adjective = 'too many';
    $buildResult = $assemblyData['BuildQty'] - $assemblyData['ActualQty'];
    if ($buildResult < 0)
    {
        $buildResult = abs($buildResult);
    }
    else
    {
        $color = '#1AC91A';
...
?>    
    <h3 style="color:<?php echo $color ?>;"><p><?php echo $buildResult.' '.$adjective; ?></h3>
...
while($buildSheet = mysqli_fetch_array($buildQuery))	{
	...
	//}
    $filename = '../img/parts/thumbs/'.$buildSheet['ID'].'Th.jpg';    
    ...
    {
         echo '<td><a href="../parts/edit_part.php?ID='.$buildSheet['ID'].'" onmouseover="document.getElementById(\''.$buildSheet['ID'].'Th\').src=\'../img/parts/thumbs/'.$buildSheet['ID'].'Th.jpg\'";  
        onmouseout="document.getElementById(\''.$buildSheet['ID'].'Th\').src=\'../img/FF0000.png\'">'.$buildSheet['Name'].'<img src="../img/FF0000.png" id="'.$buildSheet['ID'].'Th" style="box-shadow:5px 5px 5px #4D4D4D; margin:0 auto; zindex: 100; position: absolute;"></a></td>';
    }
    else
    {
        echo '<td><a href="../parts/edit_part.php?ID='.$buildSheet['ID'].'">'.$buildSheet['Name'].'</a></td>';
    }
	echo '<td>'.$buildSheet['Qty'].' <!--<a href="edit_quantity.php?ID='.str_replace(' ','',$buildSheet['assemblyBuildID']).'">Edit</a>--></td>
