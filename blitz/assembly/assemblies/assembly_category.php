...
$catID = $_GET['ID'];
$currentAssemblyQuery = mysqli_query($mysqli, "SELECT * FROM assemblies WHERE category = $catID ORDER BY Name");
...
<script>
    ...
    function processChange(id, currentBuildVal){// ActualQty*****************
    ...
    <?php
        while($assemblyList = mysqli_fetch_array($currentAssemblyQuery))	{
			echo '<tr>
                ...
                if ($assemblyList['BuildQty'] <= 0)
                {
                 echo '<td style="text-align:center;font-size:24px;font-style:bold;"><input type="text" id="aQty'.$assemblyList['ID'].'" size="2" value="'.$assemblyList['ActualQty'].'" disabled onchange="processChange('.$assemblyList['ID'].','.$assemblyList['BuildQty'].')"></td>';
                }
                else
                {
                   echo '<td style="text-align:center;font-size:24px;font-style:bold;"><input type="text" style="background-color:#FFFF8C;" id="aQty'.$assemblyList['ID'].'" size="2" value="'.$assemblyList['ActualQty'].'" onchange="processChange('.$assemblyList['ID'].','.$assemblyList['BuildQty'].')"></td>';
