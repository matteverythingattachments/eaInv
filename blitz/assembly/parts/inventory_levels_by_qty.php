...
if(isset($_GET['thickness']))	{
	$inventoryQuery = mysqli_query($mysqli,"SELECT ID, Name, Description, Thickness, Qty, MinQty, MaxQty, ReorderValue, Location, round(MinQty/Qty, 3) AS Sev_In FROM parts WHERE Thickness = $_GET[thickness] AND is_tube = 'n' ORDER BY Qty");
}
else	{
...
while($thicknessMenu = mysqli_fetch_array($thicknessQuery))	{
 ...
 include('csvimporttomin.php'); 
 include('csvimporttomax.php');
